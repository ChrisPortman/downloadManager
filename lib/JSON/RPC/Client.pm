...
sub new {
    ...
    #Find what encodings we can support (e.g. gzip)
...
sub call {
    ...
    #Support cookies in case of server implementations that still manage auth
    #outside of the API and do it with sessions.  Needs to be looked at each call.
    $self->ua->default_header( 'Cookie' => $self->{'cookie'} ) if $self->{'cookie'};
    ...
    if ($result->is_success) {
        ...
        my $retObj;
        if ($service) {
            $retObj = JSON::RPC::ServiceObject->new($result, $self->json);
        }
        else {
            $retObj = JSON::RPC::ReturnObject->new($result, $self->json);
        }
        
        #Look for a cookie and store if there is one
        my $cookie = $retObj->response->header('set-cookie');
...
BEGIN {
    for my $method (qw/is_success content jsontext version response/) {
        eval qq|
            sub $method {
                \$_[0]->{$method} = \$_[1] if defined \$_[1];
                \$_[0]->{$method};
...
sub new {
    ...
    # Decode the content (gziped or other compression)
    my $content = ( $json || JSON->new->utf8 )->decode( $response->decoded_content );
